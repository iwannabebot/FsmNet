...
- `FiniteStateMachine<TState, TContext>`: The runtime FSM.
- `FiniteStateMachineBuilder<TState, TContext>`: Fluent builder for FSM definitions.
- `TransitionRegistry<TContext>`: Register reusable conditions and side effects.
- `EnumStateMachineDefinition<TState, TContext>`: FSM definition object.
- `ITransition<TContext>`: Represents a transition between states.
...
var loaded = JsonSerializer.Deserialize<SerializableStateMachine>(json);
```
